...
vector <string> explode_raw(const string &delimiter, const string &explodeme);

vector <string> explode_raw(const string &delimiter, const string &str) {
    vector <string> arr;
...
vector<string> explode_server_data( const string &delimiter, const string &explodeme);

vector<string> explode_server_data( const string &delimiter, const string &str)
{
    vector<string> arr;
...
static UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    const RPCHelpMan help{"decoderawtransaction",
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) nexalt address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            RPCHelpMan{"decodepsbt",
                "\nReturn a JSON object representing the serialized, base64-encoded partially signed Nexalt transaction.\n",
                {
            ...
            "          \"hex\" : \"hex\",            (string) The hex\n"
            "          \"type\" : \"pubkeyhash\",    (string) The type, eg 'pubkeyhash'\n"
            "          \"address\" : \"address\"     (string) Nexalt address if there is one\n"
            "        }\n"
            "      },\n"
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            RPCHelpMan{"combinepsbt",
                "\nCombine multiple partially signed Nexalt transactions into one transaction.\n"
...
static UniValue addmlckeyraw(const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
                "addmlckeyraw \"txid\" ( verbose \"blockhash\" )\n"
                ...
                "       \"value\" : x.xxx,            (numeric) The value in " + CURRENCY_UNIT + "\n"
                                                                                                 ...
                                                                                                 "\nExamples:\n"
                + HelpExampleCli("addmlckeyraw", "\"mytxid\"")
                + HelpExampleCli("addmlckeyraw", "\"mytxid\" true")
                + HelpExampleRpc("addmlckeyraw", "\"mytxid\", true")
                + HelpExampleCli("addmlckeyraw", "\"mytxid\" false \"myblockhash\"")
                + HelpExampleCli("addmlckeyraw", "\"mytxid\" true \"myblockhash\"")
        );
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++) {
        ...
        std::vector <std::string> no_keys = explode_raw(" ", asmstring);

        if (!is_op_return_trans) {
            if (no_keys[0] == "OP_RETURN") {
                hexconverted = hexToString(no_keys[1]);
                ...
                if (mlc == "MLC") {
                    if(KoT == "K"){
                        ...
                        delete db;
                        if(valueToCheck==""){
                            //this key is not already saved
                            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "This key is Not Valid please add a valid key");
                        }else{
                            valueToSave = key;
            ...
            for (const CTxDestination& addr : addresses) {
                std::cout<<"address= "<<EncodeDestination(addr)<<"\n";
            }*/
        }
    ...
    std::vector <COutput> vecOutputs;
    {
        ...
        pwallet->AvailableCoins(*locked_chain,vecOutputs, !include_unsafe, nullptr, nMinimumAmount, nMaximumAmount, nMinimumSumAmount,
                                nMaximumCount, nMinDepth, nMaxDepth);
    }

    //getting list unspent
    ...
    for (int i = 0; i < 2; i++) {
        if (i == 0) {
            //std::string datatostore = request.params[0].get_str();
            std::string datatostore = key ;

            std::string mlcDataStore = "MLCK" + datatostore + valueToSave;
            std::string hexDataToStore = stringToHex(mlcDataStore);
...
//raw transaction created and now signing raw transactiomn
    ...
    for (unsigned int i = 0; i < mtx.vin.size(); i++) {
        ...
        // Only sign SIGHASH_SINGLE if there's a corresponding output:
        if (!fHashSingle || (i < mtx.vout.size())) {
            ProduceSignature(*pwallet, MutableTransactionSignatureCreator(&mtx, i, amount, nHashType), prevPubKey,
                             sigdata);
        }
        UpdateInput(txin, sigdata);
