...
UniValue generateBlocks(std::shared_ptr<CReserveScript> coinbaseScript, int nGenerate, uint64_t nMaxTries, bool keepScript, std::string wallet_name)
{
    ...
    while (nHeight < nHeightEnd && !ShutdownRequested())
    {
        ...
        std::unique_ptr<CBlockTemplate> pblocktemplate(BlockAssembler(Params()).CreateNewBlock(coinbaseScript->reserveScript , mlc));
...
static UniValue generatetoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "2. address      (string, required) The address to send the newly generated nexalt to.\n"
...
static UniValue getblocktemplate(const JSONRPCRequest& request)
{
        ...
        fLastTemplateSupportsSegwit != fSupportsSegwit)
    {
        ...
        pblocktemplate = BlockAssembler(Params()).CreateNewBlock(scriptDummy,wallet_name,fSupportsSegwit);
