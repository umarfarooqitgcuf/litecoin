...
UniValue generateBlocks(std::shared_ptr<CReserveScript> coinbaseScript, int nGenerate, uint64_t nMaxTries, bool keepScript,std::string wallet_name)
{
    ...
    {
        std::string mlc = "ok**"+ wallet_name;
        std::unique_ptr<CBlockTemplate> pblocktemplate(BlockAssembler(Params()).CreateNewBlock(coinbaseScript->reserveScript , mlc,true));
...
static UniValue generatetoaddress(const JSONRPCRequest& request)
{
        ...
        throw std::runtime_error(
            RPCHelpMan{"generatetoaddress",
                "\nMine blocks immediately to a specified address (before the RPC call returns)\n",
                {
                    ...
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The address to send the newly generated nexalt to."},
            ...
            + "If you are running the nexalt core wallet, you can get a new address to send the newly generated nexalt to with:\n"
    ...
    return generateBlocks(coinbaseScript, nGenerate, nMaxTries, false , wallet_name);
}
...
static UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Nexalt is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Nexalt is downloading blocks...");
    ...
    int i = 0;
    UniValue cBEntry(UniValue::VOBJ);
    for (const auto& it : pblock->vtx) {
        ...
        if (tx.IsCoinBase()){
            ...
            cBEntry.pushKV("depends", deps);

            //if(t == 0){
            int index_in_template = i - 1;
            cBEntry.pushKV("fee", pblocktemplate->vTxFees[index_in_template]);
            //}else{
            //  entry.pushKV("fee", 0);
            //}
    ...
    aMutable.push_back("coinbasetxn");
    ...
    result.pushKV("coinbasetxn", cBEntry);
...
bool IsSponsorKeySaved(std::string wallet_name){
