...
void ProcessMessageDarksend(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, bool& isDarksend) {
    ...
    if (strCommand == "dsf") { //DarkSend Final tx
        ...
        if (pfrom->nVersion < darkSendPool.MIN_PEER_PROTO_VERSION_DARK_SEND) {
            return;
        }
        ...
        if (pfrom->nVersion < darkSendPool.MIN_PEER_PROTO_VERSION_DARK_SEND) {
            ...
            g_connmanM->PushMessage(pfrom, msgMaker.Make("dssu", darkSendPool.sessionID, darkSendPool.GetState(), darkSendPool.GetEntriesCount(), MASTERNODE_REJECTED, strError));
            return;
        }
        ...
        if (mn == -1) {
            ...
            g_connmanM->PushMessage(pfrom, msgMaker.Make("dssu", darkSendPool.sessionID, darkSendPool.GetState(), darkSendPool.GetEntriesCount(), MASTERNODE_REJECTED,
                                                         strError));
        ...
        if (darkSendPool.sessionUsers == 0) {
            if (vecMasternodes[mn].nLastDsq != 0 &&
                vecMasternodes[mn].nLastDsq + CountMasternodesAboveProtocol(darkSendPool.MIN_PEER_PROTO_VERSION_DARK_SEND) / 5 > darkSendPool.nDsqCount) {
                //LogPrintf("dsa -- last dsq too recent, must wait. %s \n", vecMasternodes[mn].addr.ToString().c_str());
                std::string strError = _("Last Darksend was too recent.");

                g_connmanM->PushMessage(pfrom, msgMaker.Make("dssu", darkSendPool.sessionID, darkSendPool.GetState(), darkSendPool.GetEntriesCount(),
                                                             MASTERNODE_REJECTED, strError));
        ...
        if (!darkSendPool.IsCompatibleWithSession(nDenom, txCollateral, error)) {
            ...
            g_connmanM->PushMessage(pfrom, msgMaker.Make("dssu", darkSendPool.sessionID, darkSendPool.GetState(), darkSendPool.GetEntriesCount(), MASTERNODE_ACCEPTED, error));
            return;
        }
    }
        ...
        if (dsq.ready) {
            ...
            g_connmanM->PushMessage(pfrom ,msgMaker.Make("dsq"));
            dsq.time = GetTime();
        }
        ...
        if (pfrom->nVersion < darkSendPool.MIN_PEER_PROTO_VERSION_DARK_SEND) {
            ...
            g_connmanM->PushMessage(pfrom, msgMaker.Make("dssu", darkSendPool.sessionID, darkSendPool.GetState(),
        ...
        if (!darkSendPool.IsSessionReady()) {
            ...
            g_connmanM->PushMessage(pfrom, msgMaker.Make("dssu", darkSendPool.sessionID, darkSendPool.GetState(), darkSendPool.GetEntriesCount(), MASTERNODE_REJECTED, error));
...
int GetInputDarksendRounds(CTxIn in, int rounds) {
    ...
    const CWalletTx* wtx = pwallet->GetWalletTx(in.prevout.hash);
    if (wtx != NULL) {
        // bounds check
        ...
        if (rounds == 0 && !pwallet->IsDenominatedAmount(wtx->tx->vout[in.prevout.n].nValue)) return -2; //NOT DENOM

        bool found = false;
        for (CTxOut out : wtx->tx->vout) {
            found = pwallet->IsDenominatedAmount(out.nValue);
...
void CDarkSendPool::Check() {
    ...
    if (state == POOL_STATUS_FINALIZE_TRANSACTION && finalTransaction.vin.empty() && finalTransaction.vout.empty()) {
        ...
        if (fMasterNode) {
            ...
            //RelayDarkSendFinalTransaction(sessionID, CTransaction(txNew));
        }
    }
    ...
    if (state == POOL_STATUS_SIGNING && SignaturesComplete()) {
        ...
        CWalletTx txNew = CWalletTx(pwallet, MakeTransactionRef(std::move(finalTransaction)));
        ...
        LOCK2(cs_main, pwallet->cs_wallet);
        {
            if (fMasterNode) { //only the main node is master atm
                ...
                CValidationState state;
                if (!txNew.AcceptToMemoryPool(*locked_chain,maxTxFee,state)) {
                ...
                // Broadcast the transactionBroadcast the transaction to the network
        ...
        if (state == POOL_STATUS_ACCEPTING_ENTRIES) {
            for (const CTransaction& txCollateral : vecSessionCollateral) {
                ...
                if (!found && r > target) {
                    ...
                    CWalletTx wtxCollateral = CWalletTx(pwallet, MakeTransactionRef(std::move(txCollateral)));

                    // Broadcast
                    auto locked_chain = pwallet->chain().lock();
                    CValidationState state;
                    if (!wtxCollateral.AcceptToMemoryPool(*locked_chain, maxTxFee ,state)) {
                        // This must not fail. The transaction has already been signed and recorded.
                        LogPrintf("CDarkSendPool::ChargeFees() : Error: Transaction not valid");
                    }
                    wtxCollateral.RelayWalletTransaction(*locked_chain, nullptr);
                    return;
                }
            }
        }
            ...
            for (const CDarkSendEntry v : entries) {
                for (const CDarkSendEntryVin s : v.sev) {
                    if (!s.isSigSet && r > target) {
                        ...
                        CWalletTx wtxCollateral = CWalletTx(pwallet, MakeTransactionRef(std::move(v.collateral)));
    ...
    if (fMasterNode) {
        ...
        for (const CTransaction& txCollateral : vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Xlt and make it unusable. To
...
void CDarkSendPool::SendDarksendDenominate(std::vector <CTxIn>& vin, std::vector <CTxOut>& vout, int64_t amount) {
    ...
    CTransactionRef tRef = MakeTransactionRef(txCollateral);
    CDarkSendEntry e;
    e.Add(vin, amount, *tRef, vout);
    myEntries.push_back(e);

    // relay our entry to the master node
    //RelayDarkSendIn(vin, amount, *tRef, vout);
    Check();
}
...
bool CDarkSendPool::SignFinalTransaction(const CTransaction& finalTransactionNew, CNode* node) {
    ...
    CTransactionRef transRef = MakeTransactionRef(std::move(finalTransactionNew));
    CMutableTransaction compareTx(*transRef);

    finalTransaction = compareTx;
    ...
    for (const CDarkSendEntry e : myEntries) {
        for (const CDarkSendEntryVin s : e.sev) {
            ...
            for (unsigned int i = 0; i < finalTransactionNew.vin.size(); i++) {
                if (finalTransactionNew.vin[i] == s.vin) {
                    mine = i;
            ...
            if (mine >= 0) { //might have to do this one input at a time?
                ...
                for (unsigned int i = 0; i < finalTransactionNew.vout.size(); i++) {
                    for (const CTxOut o : e.vout) {
                        if (finalTransactionNew.vout[i] == o) {
                            foundOutputs++;
                            nValue1 += finalTransactionNew.vout[i].nValue;
                        }
                    }
                }
                ...
                CTransactionRef signTx = MakeTransactionRef(std::move(finalTransactionNew));
                CMutableTransaction txCoinbase(*signTx);


                if (fDebug) LogPrintf("CDarkSendPool::Sign - Signing my input %i\n", mine);
                const CAmount& amount = finalTransactionNew.vout[finalTransactionNew.vin[mine].prevout.n].nValue;
                SignatureData sigdata;
                bool isSigned = ProduceSignature(*pwallet , MutableTransactionSignatureCreator(&txCoinbase, (unsigned int) mine, amount, SIGHASH_ALL), prevPubKey, sigdata); // changes scriptSig
                if (!isSigned) {
                    if (fDebug) LogPrintf("CDarkSendPool::Sign - Unable to sign my own transaction! \n");
                    // not sure what to do here, it will timeout...?
                }
                UpdateTransaction(txCoinbase, (unsigned int) mine, sigdata);

                sigs.push_back(finalTransactionNew.vin[mine]);
                //if (fDebug) LogPrintf(" -- dss %d %d %s\n", mine, (int) sigs.size(), finalTransactionNew.vin[mine].scriptSig.ToString().c_str());
            }

        }

        if (fDebug) LogPrintf("CDarkSendPool::Sign - txNew:\n%s", finalTransactionNew.ToString().c_str());
    }

    // push all of our signatures to the masternode
    if (sigs.size() > 0 && node != NULL)
        g_connmanM->PushMessage(node, msgMaker.Make("dss", sigs));
...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready) {
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeLuxAmount * COIN - pwallet->GetAnonymizedBalance();
    ...
    int64_t nBalanceNotYetAnonymized = pwallet->GetBalance() - pwallet->GetAnonymizedBalance();
    ...
    if (!pwallet->SelectCoinsDark(nValueMin, nBalanceNeedsAnonymized, vCoins, nValueIn, 0, nDarksendRounds)) {
        nValueIn = 0;
        vCoins.clear();
    ...
    if (!sessionFoundMasternode) {
        ...
        sessionTotalValue = pwallet->GetTotalValue(vCoins);
        ...
        LogPrintf("Submitting Darksend for %f XLT CENT - sessionTotalValue %d\n", fLuxSubmitted, sessionTotalValue);

        if (pwallet->GetDenominatedBalance(true, true) > 0) { //get denominated unconfirmed inputs
        ...
        if (nUseQueue > 33) {
            ...
            for (CDarksendQueue & dsq : vecDarksendQueue) {
                ...
                if (protocolVersion < MIN_PEER_PROTO_VERSION_DARK_SEND) continue;
                ...
                if (!pwallet->SelectCoinsByDenominations(dsq.nDenom, nValueMin, nBalanceNeedsAnonymized, vCoins, vCoins2, nValueIn, 0, nDarksendRounds)) {
                    LogPrintf("DoAutomaticDenominating - Couldn't match denominations %d\n", dsq.nDenom);
                    continue;
                }

                std::unique_ptr<CConnman>  g_connmanM = std::unique_ptr<CConnman>(new CConnman(GetRand(std::numeric_limits<uint64_t>::max()), GetRand(std::numeric_limits<uint64_t>::max())));
                std::vector<AddedNodeInfo> vInfo = g_connmanM->GetAddedNodeInfo();
                for (const AddedNodeInfo& info : vInfo) {
                    if (!info.fConnected) {
                        const char *pszDest = info.strAddedNode.c_str();
                        // connect to masternode and submit the queue request
                        if (g_connmanM->ConnectNode(CAddress(addr, NODE_NETWORK), pszDest, true, true)) {
        ...
        while (i < 10) {
            ...
            if (vecMasternodes[i].protocolVersion < MIN_PEER_PROTO_VERSION_DARK_SEND) {
                i++;
                continue;
            }

            if (vecMasternodes[i].nLastDsq != 0 &&
                vecMasternodes[i].nLastDsq + CountMasternodesAboveProtocol(darkSendPool.MIN_PEER_PROTO_VERSION_DARK_SEND) / 5 > darkSendPool.nDsqCount) {
                i++;
                continue;
            }
            ...
            for (const AddedNodeInfo& info : vInfo) {
                if (!info.fConnected) {
                    const char *pszDest = info.strAddedNode.c_str();
                    // connect to masternode and submit the queue request
                    if (g_connmanM->ConnectNode(CAddress(vecMasternodes[i].addr, NODE_NETWORK), pszDest, true, true)) {
...
bool CDarkSendPool::PrepareDarksendDenominate() {
    ...
    std::string strError = pwallet->PrepareDarksendDenominate(0, nDarksendRounds);
...
bool CDarkSendPool::SendRandomPaymentToSelf() {
    ...
    bool success = pwallet->CreateTransaction(*locked_chain ,vecSend, wtx, reservekey, nFeeRet, nChangePos, strFail, coinControl);
    if (!success) {
    ...
    CValidationState state;
    pwallet->CommitTransaction(wtx,{},{}, reservekey,nullptr,state);
...
bool CDarkSendPool::MakeCollateralAmounts() {
    ...
    bool success = pwallet->CreateTransaction(*locked_chain,vecSend, wtx, reservekey, nFeeRet, nChangePos, strFail, coinControl, true);
    if (!success) {
        ...
        success = pwallet->CreateTransaction(*locked_chain,vecSend, wtx, reservekey, nFeeRet, nChangePos, strFail, coinControl, true);
        if (!success) {
    ...
    CValidationState state;
    if(pwallet->CommitTransaction(wtx,{},{}, reservekey,nullptr,state))
        cachedLastSuccess = chainActive.Height();
...
void ThreadCheckDarkSendPool(CConnman* connman) {
    std::string wallet_name = "";
    std::shared_ptr <CWallet> wallet = GetWallet(wallet_name);
    CWallet *const pwallet = wallet.get();
    // Make this thread recognisable as the wallet flushing thread
    RenameThread("Xlt-darksend");
    ...
    while (!ShutdownRequested()) {
        ...
        if (c % 5 == 0 && RequestedMasterNodeList < 5) {
            ...
            if (!fIsInitialDownload) {
                //LOCK(cs_vNodes);
                for (CNode * pnode : connman->vNodes) {
                    if (pnode && pnode->nVersion >= darkSendPool.MIN_PEER_PROTO_VERSION_DARK_SEND) {
                        ...
                        const CNetMsgMaker msgMaker(PROTOCOL_VERSION);
                        connman->PushMessage(pnode ,msgMaker.Make("dseg", CTxIn()));//request full mn list
                        connman->PushMessage(pnode ,msgMaker.Make("mnget"));//sync payees
        ...
        if (c % 60 * (nLiquidityProvider + 1) == 0) {
            if (nLiquidityProvider != 0) {
                ...
                if (nRand == 50 + nLiquidityProvider && pwallet->GetAverageAnonymizedRounds() > 8) {
                    darkSendPool.SendRandomPaymentToSelf();
                    int nLeftToAnon = ((pwallet->GetBalance() - pwallet->GetAnonymizedBalance()) / COIN) - 3;
