...
void ProcessInstantX(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, bool& isInstantXCommand) {
    ...
    if (strCommand == "txlreq") {
        ...
        if (AcceptToMemoryPool(mempool, state, txref, &fMissingInputs,nullptr,true,0 )) {
            vector<CInv> vInv;
            vInv.push_back(inv);

            g_connmanM->PushMessage(pfrom, msgMaker.Make("inv", vInv));

            DoConsensusVote(pfrom,tx, nBlockHeight);
...
void DoConsensusVote(CNode* pfrom,CTransaction& tx, int64_t nBlockHeight) {
    if (!fMasterNode) return;

    int n = GetMasternodeRank(activeMasternode.vin, nBlockHeight, MIN_PEER_PROTO_VERSION);
...
bool ProcessConsensusVote(CConsensusVote& ctx) {
    ...
    int n = GetMasternodeRank(ctx.vinMasternode, ctx.nBlockHeight, MIN_PEER_PROTO_VERSION);
...
bool CTransactionLock::SignaturesValid() {
    ...
    for (CConsensusVote vote : vecConsensusVotes) {
        int n = GetMasternodeRank(vote.vinMasternode, vote.nBlockHeight, MIN_PEER_PROTO_VERSION);
