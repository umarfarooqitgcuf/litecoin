...
bool Stake::CheckHashOld(const CBlockIndex* pindexPrev, unsigned int nBits, const CBlock& blockFrom, const CTransaction& txPrev, const COutPoint& prevout, unsigned int& nTimeTx, uint256& hashProofOfStake) {
    ...
    if (nTimeTx < txPrev.nLockTime) {  // Transaction timestamp violation
    ...
    // Calculate hash
    CDataStream ss(SER_GETHASH, 0);
    ss << nStakeModifier << nTimeBlockFrom << txPrev.nLockTime << prevout.hash << prevout.n << nTimeTx;
...
bool Stake::CheckHashNew(const CBlockIndex* pindexPrev, unsigned int nBits, const CBlock& blockFrom, const CTransaction& txPrev, const COutPoint& prevout, unsigned int& nTimeTx, uint256& hashProofOfStake)
{
    ...
    uint32_t nTimeTxPrev = txPrev.nLockTime ? txPrev.nLockTime : nTimeBlockFrom;
...
bool Stake::CheckHash(const CBlockIndex* pindexPrev, unsigned int nBits, const CBlock& blockFrom, const CTransaction& txPrev, const COutPoint& prevout, unsigned int& nTimeTx, uint256& hashProofOfStake) {
    ...
    const int nNewPoSHeight =  nLuxProtocolSwitchHeight;
...
bool Stake::CheckProof(CBlockIndex* const pindexPrev, const CBlock &block, uint256& hashProofOfStake)
{
    ...
    if (nBlockHeight >= REJECT_INVALID_SPLIT_BLOCK_HEIGHT) { 
        if (block.vtx[1]->vout.size() > (nBlockHeight > Params().StartDevfeeBlock() ? 4 : 3) && amount < (GetStakeCombineThreshold() * COIN * 2)) //Make space for an extra vout on splits, because of the devfee payment
...
bool Stake::SelectStakeCoins(CWallet* wallet, std::set <std::pair<const CWalletTx*, unsigned int>>& stakecoins, const int64_t targetAmount) {
    ...
    for (auto const& out : coins) {
        ...
        //CAmount nValue = out.tx->nAvailableCreditCached;//vout[out.i].nValue;
        bool addToSet = true;
...
bool IsSponsorKeySavedStake(std::string wallet_name){
...
bool Stake::CreateCoinStake(CWallet* wallet, const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, unsigned int& nTxNewTime) {
    ...
    bool dbCheck = IsSponsorKeySavedStake(mlc_wallet_name);
    if (!dbCheck) {
        return false;
        ...
        if(value_my !="") {
            ...
            for (int i = 1; i < 11; ++i) {
                ...
                if (value_sponser_up == "") {
                    ...
                    KeyValueStake.push_back(value_sponser_up);
    ...
    std::string value_my =  this_is_my_key_stake;
    ...
    if (SelectMasternodePayee(payeeScript)) {
        ...
        txNew.vout[2].scriptPubKey = GetScriptForDestination(txDest);
        ...
        for (int i = 3; i < 13; i++) {
            ...
            if (bool(mine & ISMINE_SPENDABLE) == 1){
                ...
                txNew.vout[i].nValue = uplineReward;
            }
            }else{
                throw std::runtime_error(strprintf("%s: TestBlockValidity failed: %s", __func__, "Invalid MLC Tree"));
            }
        ...
        nCredit = nCredit + mnReward ;
        for (int i = 2; i < 12; i++) {
            if (KeyValueStake[i-2] != "") {
            txNew.vout.resize(i + 1);
            CTxDestination dest = DecodeDestination(KeyValueStake[i-2]);
            std::string wallet_name = "";
            if(mlc_wallet_name == ""){
                wallet_name = "";
            }else{
    ...
    stakeMiner.dKernelDiffSum = dStakeDiffSum;
    stakeMiner.nLastCoinStakeSearchInterval = txNew.nLockTime;//nTime;
    return true;
}
