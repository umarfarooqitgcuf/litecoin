...
//mlc db
    leveldb::DB *db_mlc;
    leveldb::Options options_mlc;
    options_mlc.create_if_missing = true;

    std::string std_data_dir = GetDataDir().string();
    int countKeys=0;

    //open db.
    leveldb::Status status = leveldb::DB::Open(options, std_data_dir + "/myKey", &db);
    leveldb::Status status_mlc = leveldb::DB::Open(options_mlc, std_data_dir + "/mlc", &db_mlc);
    //std::string StringKey = "StringKey";
    std::string StringKeyToShow = "StringKeyToShow";
    std::string key;
    status = db->Get(leveldb::ReadOptions(), StringKeyToShow, &key);
    leveldb::Iterator* it = db_mlc->NewIterator(leveldb::ReadOptions());
    ...
    delete db_mlc;
    delete db;
    QString qs = QString::fromLocal8Bit(key.c_str());

    ui->sponsershipkey->setText(qs);
    ui->numberofkeys->setText(QString::number(countKeys));
}
...
void RPCConsole::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool headers)
{
    ...
    if (!headers) {
        //ui->sponsershipkey->setText(QString::number(count));
