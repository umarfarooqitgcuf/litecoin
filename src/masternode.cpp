...
static const int MIN_PROTO_VERSION = MIN_PEER_PROTO_VERSION;
...
void ProcessMasternode(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, bool& isMasternodeCommand) {
    ...
    if (strCommand == "dsee") { //DarkSend Election Entry
        ...
        int mncollateral = chainActive.Height()>=30000 ? 300 : 200;
        ...
        LogPrintf("dseg - Sent Second %d masternode entries to %s\n", count, pfrom->addr.ToString().c_str());
    }
...
uint256 CMasterNode::CalculateScore(int mod, int64_t nBlockHeight) {
    ...
    uint256 hashS = ArithToUint256(hash);
    uint256 auxS = ArithToUint256(aux);

    if (!GetBlockHash(hashS, nBlockHeight)) {
        return ArithToUint256(0);
    }



    uint256 hash2 = Hash(BEGIN(hashS), END(hashS));
    uint256 hash3 = Hash(BEGIN(hashS), END(auxS));

    arith_uint256 hash2S = UintToArith256(hash2);
    arith_uint256 hash3S = UintToArith256(hash3);

    arith_uint256 rR = (hash3S > hash2S ? hash3S - hash2S : hash2S - hash3S);

    uint256 r = ArithToUint256(rR);
...
uint64_t CMasternodePayments::CalculateScore(uint256 blockHash, CTxIn& vin) {
    arith_uint256 n1 = UintToArith256(blockHash);
    arith_uint256 n2 = UintToArith256(Hash(BEGIN(n1), END(n1)));
    arith_uint256 n3 = UintToArith256(Hash(BEGIN(vin.prevout.hash), END(vin.prevout.hash)));
    arith_uint256 n4 = n3 > n2 ? (n3 - n2) : (n2 - n3);
...
void CMasternodePayments::Relay(CMasternodePaymentWinner& winner) {
    ...
    for (CNode* pnode : g_connmanM->vNodes) {
        if (pnode) {
            g_connmanM->PushMessage(pnode, msgMaker.Make("inv", vInv));
...
void CMasternodePayments::Sync(CNode* node) {
    ...
    for (CMasternodePaymentWinner& winner : vWinning)
    if (winner.nBlockHeight >= chainActive.Height() - 10 && winner.nBlockHeight <= chainActive.Height() + 20) {
        g_connmanM->PushMessage(node, msgMaker.Make("mnw", winner, a));
    }
