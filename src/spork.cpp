...
void ProcessSpork(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, bool& isSporkCommand) {
    ...
    if (strCommand == "spork") {
        ...
        while (it != mapSporksActive.end()) {
            g_connmanM->PushMessage(pfrom, msgMaker.Make("spork", it->second));
            it++;
        }
    }
}
...
bool CSporkManager::UpdateSpork(CConnman* connman, int nSporkID, int64_t nValue) {
...
void CSporkManager::Relay(CConnman* connman ,CSporkMessage& msg) {
    std::unique_ptr<CConnman>  g_connmanM = std::unique_ptr<CConnman>(new CConnman(GetRand(std::numeric_limits<uint64_t>::max()), GetRand(std::numeric_limits<uint64_t>::max())));
    const CNetMsgMaker msgMaker(PROTOCOL_VERSION);
    ...
    if (pnode)
        connman->PushMessage(pnode, msgMaker.Make("inv", vInv));
    }
}
