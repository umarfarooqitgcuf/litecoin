...
size_t CConnman::SocketSendData(CNode *pnode) const {
    ...
    while (it != pnode->vSendMsg.end()) {
        const auto &data = *it;
        //std::cout << "Message s: " << reinterpret_cast<const char *>(data.data()) << "\n";
        ...
        int nBytes = 0;
        {
            ...
            //std::cout << "Message sent: " << reinterpret_cast<const char *>(data.data()) << ", Offset: " << pnode->nSendOffset << "\n";
        }
        if (nBytes > 0) {
    ...
    std::vector <NodeEvictionCandidate> vEvictionCandidates;
    {
        ...
        for (const CNode *node : vNodes) {
            ...
            NodeEvictionCandidate candidate = {node->GetId(), node->nTimeConnected, node->nMinPingUsecTime,
                                               ...
                                               node->fRelayTxes, node->pfilter != nullptr, node->addr,
                                               node->nKeyedNetGroup};
        ...
        for (CNode *pnode : vNodesCopy) {
            ...
            if (recvSet || errorSet) {
                ...
                int nBytes = 0;
                {
                    ...
                    //LogPrintf( "receiving %u \n", pchBuf);
                    //std::cout << "Receiving: " << pchBuf <<"\n";
...
void CConnman::PushMessage2(CNode *pnode, CSerializedNetMsg &&msg) {
    ...
    size_t nBytesSent = 0;
    {
        ...
        std::cout << "Data to send: " <<  nTotalSize << ": " << "\n";
...
static void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "Nexalt " + FormatFullVersion();
...
void
CConnman::OpenNetworkConnection(const CAddress &addrConnect, bool fCountFailure, CSemaphoreGrant *grantOutbound,
...
void
CConnman::AddNewAddresses(const std::vector <CAddress> &vAddr, const CAddress &addrFrom, int64_t nTimePenalty) {
...
CNode::CNode(NodeId
             idIn, ServiceFlags
             nLocalServicesIn, int
             nMyStartingHeightIn, SOCKET
             hSocketIn,
             const CAddress &addrIn, uint64_t
             nKeyedNetGroupIn, uint64_t
             nLocalHostNonceIn,
...
void CNode::AskFor(const CInv &inv) {
    ...
    LogPrint(BCLog::NET, "askfor %s  %d (%s) peer=%d\n", inv.ToString(), nRequestTime,
             FormatISO8601Time(nRequestTime / 1000000), id);
...
CSipHasher
CConnman::GetDeterministicRandomizer(uint64_t
                                     id) const {
...
uint64_t CConnman::CalculateKeyedNetGroup(const CAddress &ad) const {
    ...
    return GetDeterministicRandomizer(RANDOMIZER_ID_NETGROUP).Write(vchNetGroup.data(),
                                                                    vchNetGroup.size()).Finalize();
}
