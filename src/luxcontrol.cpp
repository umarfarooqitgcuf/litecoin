...
/** Nexalt cookie size (from control-spec.txt) */
static const int LUX_COOKIE_SIZE = 32;
/** Size of client/server nonce for SAFECOOKIE */
static const int LUX_NONCE_SIZE = 32;
/** For computing serverHash in SAFECOOKIE */
static const std::string LUX_SAFE_SERVERKEY = "Nexalt safe cookie authentication server-to-controller hash";
/** For computing clientHash in SAFECOOKIE */
static const std::string LUX_SAFE_CLIENTKEY = "Nexalt safe cookie authentication controller-to-server hash";
...
auto& _1 = boost::placeholders::_1;
auto& _2 = boost::placeholders::_2;
...
bool LuxControlConnection::Connect(const std::string &target, const ConnectionCB& _connected, const ConnectionCB&  _disconnected)
{
    ...
    if (evutil_parse_sockaddr_port(target.c_str(),
        ...
        LogPrintf("nexalt: Error parsing socket address %s\n", target);
        return false;
    }
    ...
    if (bufferevent_socket_connect(b_conn, (struct sockaddr*)&connect_to_addr, connect_to_addrlen) < 0) {
        LogPrintf("nexalt: Error connecting to address %s\n", target);
        return false;
    }
    return true;
}
    ...
    reconnect_timeout(RECONNECT_TIMEOUT_START)
{
    ...
    if (!reconnect_ev)
        LogPrintf("nexalt: Failed to create event for reconnection: out of memory?\n");
    ...
    if (!conn.Connect(_target, boost::bind(&LuxController::connected_cb, this, _1),
        ...
        LogPrintf("nexalt: Initiating connection to Nexalt control port %s failed\n", _target);
    }
...
void LuxController::add_onion_cb(LuxControlConnection& _conn, const LuxControlReply& reply)
{
    if (reply.code == 250) {
        ...
        if (service_id.empty()) {
            LogPrintf("nexalt: Error parsing ADD_ONION parameters:\n");
            for (const std::string &s : reply.lines) {
                LogPrintf("    %s\n", SanitizeString(s));
            }
            return;
        ...
        LogPrintf("nexalt: Got service ID %s, advertising service %s\n", service_id, service.ToString());
        if (WriteBinaryFile(GetPrivateKeyFile(), private_key)) {
            //LogPrint("lux", "lux: Cached service private key to %s\n", GetPrivateKeyFile());
        } else {
            LogPrintf("nexalt: Error writing service private key to %s\n", GetPrivateKeyFile());
        }
        AddLocal(service, LOCAL_MANUAL);
        // ... onion requested - keep connection open
    } else if (reply.code == 510) { // 510 Unrecognized command
        LogPrintf("nexalt: Add onion failed with unrecognized command (You probably need to upgrade Nexalt)\n");
    } else {
        LogPrintf("nexalt: Add onion failed; error code %d\n", reply.code);
    }
}
...
void LuxController::auth_cb(LuxControlConnection& _conn, const LuxControlReply& reply)
{
    if (reply.code == 250) {
        ...
        if (gArgs.GetArg("-onion", "") == "") {
            CService resolved;
            //assert(LookupNumeric("127.0.0.1", 9050));
        ...
        LogPrintf("nexalt: Authentication failed\n");
    }
}
...
void LuxController::authchallenge_cb(LuxControlConnection& _conn, const LuxControlReply& reply)
{
    if (reply.code == 250) {
        ...
        if (l.first == "AUTHCHALLENGE") {
            ...
            if (m.empty()) {
                LogPrintf("nexalt: Error parsing AUTHCHALLENGE parameters: %s\n", SanitizeString(l.second));
                return;
            }
            ...
            if (serverNonce.size() != 32) {
                LogPrintf("nexalt: ServerNonce is not 32 bytes, as required by spec\n");
                return;
            }
...
void LuxController::protocolinfo_cb(LuxControlConnection& _conn, const LuxControlReply& reply)
{
    if (reply.code == 250) {
         ...
         *   password: "password"
         */
        std::string luxpassword = gArgs.GetArg("-luxpassword", "");
...
{
    LuxController ctrl(gBase, gArgs.GetArg("-luxcontrol", DEFAULT_LUX_CONTROL));
