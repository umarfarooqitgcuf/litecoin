 ...
 */
static const char* BITCOIN_PID_FILENAME = "nexaltd.pid";
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/nexalt-project/Nexalt>";
    const std::string URL_WEBSITE = "<https://nexalt.org>";

    return CopyrightHolders(strprintf(_("Copyright (C) %i-%i"), COPYRIGHT_YEAR_START, COPYRIGHT_YEAR) + " ") + "\n" +
           "\n" +
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if nexalt is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if nexalt is started while in a temporary directory.\n",
